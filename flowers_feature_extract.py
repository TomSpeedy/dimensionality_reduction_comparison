...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train):
        ...
        output = tf.keras.layers.Dense(len(fd.FLOWERS.LABELS), activation = tf.nn.softmax, kernel_regularizer = tf.keras.regularizers.l2(args.l2))(output)
...
def main(args: argparse.Namespace) -> None:
    ...
    def train_augment(image: tf.Tensor, label: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        image = tf.image.resize_with_crop_or_pad(image, fd.FLOWERS.H + 100, fd.FLOWERS.W + 100)
        image = tf.image.resize(image, [generator.uniform([], fd.FLOWERS.H, fd.FLOWERS.H + 100 + 1, dtype=tf.int32),
                                        generator.uniform([], fd.FLOWERS.W, fd.FLOWERS.H + 100 + 1, dtype=tf.int32)])
        image = tf.image.crop_to_bounding_box(
            image, target_height=fd.FLOWERS.H, target_width=fd.FLOWERS.W,
            offset_height=generator.uniform([], maxval=tf.shape(image)[0] - fd.FLOWERS.H + 1, dtype=tf.int32),
            offset_width=generator.uniform([], maxval=tf.shape(image)[1] - fd.FLOWERS.W + 1, dtype=tf.int32),
        )
        return image, label    
